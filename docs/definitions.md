...
| **Decision** | A commitment in `text`, with a required `choice` and the `alternatives` considered | `adopted`, `revoked` |
...
- `alternatives`: options considered but not chosen; empty when there were none.
